...
public class Crypt {
    ...
    static char[][] KS = new char[16][48];

    /* The E bit-selection table. */
    static char[] E = new char[48];
    static char e2[] = {
        32,  1,  2,  3,  4,  5,
        4,  5,  6,  7,  8,  9,
        8,  9, 10, 11, 12, 13,
     ...
     **************************************************************************/
    private char[] crypt(char[] pw, char[] salt) {
        int i, j, temp;
        char c;
	char[] block = new char[66];  /* 1st store key, then results */
        char[] iobuf = new char[16];      /* encrypted results */

        for(i = 0; i < 66; i++)
            block[i] = 0;

        /* break pw into 64 bits */
        for(i = 0, c = pw[i]; c>0 && (i < 64); i++) {
            for(j = 0; j < 7; j++, i++)
	...
	char[] salt = args[1].toCharArray();
	Crypt c = new Crypt();
	char[] trip = c.crypt(pass, salt);
	System.out.println(new String(trip));
    }
}