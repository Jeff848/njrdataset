...
public class LocalConcurrentScenarioGenerator {
	...
	static String SCENARIO = "local-concurrent";
	...
	private static void run(Properties properties, int workersNo, int tasksNo, long secondJobDelay, int replica)
		...
		brokerA.setPeerId(peer.getId());
		grid.addObject(brokerA);
		evProxy.add(new EventSpec(BrokerEvents.BROKER_UP, 0, brokerA.getId()));
		
		peer.addBroker(brokerA.getId());
		
		Broker brokerB = new Broker();
		brokerB.setId("brokerB");
		brokerB.setPeerId(peer.getId());
		grid.addObject(brokerB);
		evProxy.add(new EventSpec(BrokerEvents.BROKER_UP, 0, brokerB.getId()));
		
		peer.addBroker(brokerB.getId());
		...
		evProxy.add(new EventSpec(BrokerEvents.ADD_JOB, 0, brokerA.getId() + " " + jsonObject.toString()));
		evProxy.add(new EventSpec(BrokerEvents.ADD_JOB, secondJobDelay, brokerB.getId() + " " + jsonObject.toString()));
